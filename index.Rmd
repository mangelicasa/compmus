...
[Plantasia](https://open.spotify.com/intl-es/track/2XV5CUyw7gPVi18d718D4f?si=e5a10490b1f7464d) I was surprised to see the average tempo is 350 bpm, which really surprised me. It seems tho that it is just taking multiples of the actual tempo.

As we can see in the second graph the actual tempo is arounf 88bpm, which is closer to a slower tempo that I expected for music for growing plants.
...
### Hold the tempo
```{r}
tempo_hist <- ggplot(wrapped, aes(x = factor(tempo, fill = category))) +
  geom_bar(stat = "count", position = "identity", alpha = 0.7) +
  facet_wrap(~ category, nrow = 3, scales = "free_x") +
  theme(legend.position = "none") +
  scale_fill_brewer(palette = "PiYG") +
  labs(title = "Histogram of Tempo of Songs for Each Year",
       x = "Tempo",
       y = "Frequency") +
  theme_minimal() +
  theme(
